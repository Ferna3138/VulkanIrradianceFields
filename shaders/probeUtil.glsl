...
vec2 normalised_oct_coord(ivec2 fragCoord, int probe_side_length) {
...
vec2 texture_coord_from_direction(vec3 dir, int probe_index, int full_texture_width, int full_texture_height, int probe_side_length) {
  ...
  vec2 normalised_oct_coord = oct_encode(normalize(dir));
  // Map it to [0,1]
  vec2 normalised_oct_coord_zero_one = (normalised_oct_coord * 0.5) + 0.5f;
  ...
  vec2 oct_coord_normalized_to_texture_dimensions = (normalised_oct_coord_zero_one * float(probe_side_length)) / vec2(float(full_texture_width), float(full_texture_height));
